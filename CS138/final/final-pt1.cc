...
Justifier::Justifier(int MaxLineLength, ifstream &instream, ofstream &outstream)
    : MaxLineLength(MaxLineLength), instream(instream), outstream(outstream),
      curChunk("") {}
...
string Justifier::getNextInputChunk()
{
    ...
    // otherwise take the first `MaxLineLength` characters from the current chunk
    // as the next chunk
...
TokenBasedJustifier::TokenBasedJustifier(int MaxLineLength, ifstream &instream, ofstream &outstream)
    : Justifier(MaxLineLength, instream, outstream), curLine(""),
      boldfaceOn(false), beginningOfLine(true) {}

TokenBasedJustifier::~TokenBasedJustifier() {}

// convert each character in the string to upper case and return the new
// concatenated string
...
// set `boldFaceOn` when encountering <bf> or </bf>
...
string TokenBasedJustifier::getNextLine()
{
    ...
    if (this->curLine.size() > 0)
    {
        ...
        // otherwise grab all the chars from the current line as the first token in
        // the line
        next_line.push_back(this->curLine);
        remaining_chars -= this->curLine.size();
        this->curLine = "";
    }
    // otherwise if the current line is empty then grab the next input chunk,
    // hyphenate if necessary and add that as the first token in the line
    else
    {
        ...
        // skip <br> tokens and start a new line
        if (next_token == "<br>")
        {
    ...
    // as long as we have space in the current line keep grabbing more tokens from
    // the file and adding them to the current line save any tokens that go beyond
    // the max length of the line to `curLine`
...
RaggedRightJustifier::RaggedRightJustifier(int MaxLineLength,
                                           ifstream &instream,
                                           ofstream &outstream)
    : TokenBasedJustifier(MaxLineLength, instream, outstream) {}

RaggedRightJustifier::~RaggedRightJustifier() {}

// the ragged right justifier is already justified as is :)
string RaggedRightJustifier::justifyLine(const string &line) { return line; }
...
// use the parent's constructor to initialize this child class
RaggedLeftJustifier::RaggedLeftJustifier(int MaxLineLength, ifstream &instream,
                                         ofstream &outstream)
    : TokenBasedJustifier(MaxLineLength, instream, outstream) {}
...
{
public:
    CentredJustifier(int MaxLineLength, ifstream &instream, ofstream &outstream);
...
// use the parent's constructor to initialize this child class
CentredJustifier::CentredJustifier(int MaxLineLength, ifstream &instream,
                                   ofstream &outstream)
    : TokenBasedJustifier(MaxLineLength, instream, outstream) {}
...
{
public:
    SmoothJustifier(int MaxLineLength, ifstream &instream, ofstream &outstream);
...
// use the parent's constructor to initialize this child class
SmoothJustifier::SmoothJustifier(int MaxLineLength, ifstream &instream,
                                 ofstream &outstream)
    : TokenBasedJustifier(MaxLineLength, instream, outstream) {}
...
{
public:
    LineBasedJustifer(int MaxLineLength, ifstream &instream, ofstream &outstream);
...
LineBasedJustifer::LineBasedJustifer(int MaxLineLength, ifstream &instream,
                                     ofstream &outstream)
    : Justifier(MaxLineLength, instream, outstream) {}
...
string LineBasedJustifer::getNextLine()
{
    ...
    // otherwise if the current line is empty then grab the next input chunk,
    // hyphenate if necessary and return it
    else
    {
        string next_token = this->getNextInputChunk();
...
VerbatimJustifier::VerbatimJustifier(int MaxLineLength, ifstream &instream,
                                     ofstream &outstream)
    : LineBasedJustifer(MaxLineLength, instream, outstream) {}
...
int main(int argc, char *argv[])
{
    ...
    const string maxLineLengthTooShort =
        "Error, MaxLineLength must be at least " +
        ::to_string(Justifier::MinLineLength);
    ...
    const string usageMsg =
        "Usage: justify [-m <MaxLineLen>] [-s [rr|rl|c|s|v]] <infile> <outfile>";
    ...
    {
        // make sure that the first optional parameter is either "-m" and the second
        // optional parameter is "-s"
        if (args[1] == "-m")
        {
            int line_length = stoi(args[2]);
            if (line_length < Justifier::MinLineLength)
