...
public class StripesPMI extends Configured implements Tool {
  ...
  public static final class CooccurrenceReducer extends Reducer<Text, HMapStIW, Text, HashMapWritable> {
    ...
    public void reduce(Text key, Iterable<HMapStIW> values, Context context)
      ...
      HashMapWritable<Text, PairOfFloatInt> map2 = new HashMapWritable<Text, PairOfFloatInt>();

      float n_lines = (float) word_counts.get("*");
      float p_x = (float) word_counts.get(key.toString()) / n_lines;
      ...
      for (String word : map.keySet()) {
        if (map.get(word) >= threshold) {
          non_empty = true;
          float p_y = (float) word_counts.get(word) / n_lines;
          float p_x_y = (float) map.get(word) / n_lines;
          float pmi = (float) Math.log10(p_x_y / (p_x * p_y));
          map2.put(new Text(word), new PairOfFloatInt(pmi, map.get(word)));
        }
      }
  ...
  private static final class CooccurrencePartitioner extends Partitioner<Text, HMapStIW> {
    @Override
