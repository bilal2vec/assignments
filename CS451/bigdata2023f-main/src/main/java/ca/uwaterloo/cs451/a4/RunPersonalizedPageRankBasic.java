 ...
 */
public class RunPersonalizedPageRank extends Configured implements Tool {
  private static final Logger LOG = Logger.getLogger(RunPersonalizedPageRank.class);
  ...
  public static void main(String[] args) throws Exception {
    ToolRunner.run(new RunPersonalizedPageRank(), args);
  }

  public RunPersonalizedPageRank() {}
  ...
  public int run(String[] args) throws Exception {
    ...
    LOG.info(" - source nodes: " + source_nodes);

    String[] source_nodes_raw = source_nodes.split("\\,");

    // Iterate PageRank.
    for (int i = s; i < e; i++) {
      iteratePageRank(i, i + 1, basePath, n, source_nodes_raw, useCombiner, useInmapCombiner);
    }
  ...
  // Run each iteration.
  private void iteratePageRank(int i, int j, String basePath, int numNodes, String[] source_nodes_raw,
  ...
  private float phase1(int i, int j, String basePath, int numNodes,
    ...
    job.setJarByClass(RunPersonalizedPageRank.class);
