...
/*
	Pre: q is non-NULL and valid, dest is big enough
	Post: dest contains a serialization starting at *dest.
	Return is number of characters written to dest
*/
int serialize_queue_to_dest(char *dest, queue *q, int (*serialize_node_to_dest)(char* d, node *)) {
	node *cur = q->head;

	int bytes_written = 0;
	int node_serialization_length = 0;

	while (cur != NULL) {
		node_serialization_length = serialize_node_to_dest(dest, cur);
		dest += node_serialization_length * sizeof(char);
		bytes_written += node_serialization_length;
...
// and move it out of the 'from' queue and enqueue it to the 'to' queue
