...
posn find_blank(puzzle *puz)
{
    ...
    */
    ...
    // count # cells on board contain numbers
    int boxes_full = 0;
    for (int i = 0; i < puz->boardSize; i++)
    {
        entry *row = puz->board[i];
    ...
    // if all cells on boards contain numbers or list of constraints is empty
    if ((puz->constraintsLen == 0) || (boxes_full == (puz->boardSize * puz->boardSize)))
    {
        posn pos = {-2, -2};
        return pos;
    }

    // get string in the first constraint
    char first_constraint = puz->constraints[0].letter;

    // if the string in the first constraint has only guesses
    bool first_constraint_only_guesses = false;
    int n_guess = 0;
    for (int i = 0; i < puz->boardSize; i++)
    {
        entry *row = puz->board[i];
        ...
        {
            guess temp = row[j].g;
            if (temp.letter == first_constraint)
            {
                n_guess += 1;

                if (n_guess == 1)
                {
                    first_constraint_only_guesses = true;
                }

                if (temp.number == 0)
                {
                    first_constraint_only_guesses = false;
                }
            }
        }
    }

    if (first_constraint_only_guesses)
    {
        posn pos = {-1, -1};
        return pos;
    }
    ...
    for (int i = 0; i < puz->boardSize; i++)
    {
        ...
        for (int j = 0; j < puz->boardSize; j++)
        {
            ...
            if (val.letter == first_constraint)
            {
                // if (val.number == 0)
                // {
                posn temp = {j, i};
...
numArr used_in_row(puzzle *puz, posn pos)
{
    ...
    int *used_in_row = malloc(count * sizeof(int));
    ...
    for (int j = 0; j < puz->boardSize; j++)
    {
        ...
        if (num != 0)
        {
            used_in_row[counter] = num;
            counter += 1;
        }

        if (guessnum != 0)
        {
            used_in_row[counter] = guessnum;
            counter += 1;
        }
    }

    qsort(used_in_row, count, sizeof(int), comparet);

    numArr temp = {used_in_row, count};
...
numArr used_in_col(puzzle *puz, posn pos)
{
    ...
    int *used_in_col = malloc(count * sizeof(int));
    ...
    for (int i = 0; i < puz->boardSize; i++)
    {
        for (int j = 0; j < puz->boardSize; j++)
        {
            if (j == pos.x)
            {
                ...
                if (num != 0)
                {
                    used_in_col[counter] = num;
                    counter += 1;
                }

                if (guessnum != 0)
                {
                    used_in_col[counter] = guessnum;
                    counter += 1;
                }
            }
        }
    ...
    qsort(used_in_col, count, sizeof(int), comparet);

    numArr temp = {used_in_col, count};
...
numArr available_vals(puzzle *puz, posn pos)
{
    ...
    numArr row = used_in_row(puz, pos);
    numArr col = used_in_col(puz, pos);

    int largest_val = puz->boardSize + 1;
    int largest_idx = puz->boardSize;

    int *vals = malloc(sizeof(int) * largest_idx);
    ...
    int *vals2 = malloc(sizeof(int) * count);
    int counter = 0;
    for (int i = 0; i < largest_idx; i++)
    {
        if (vals[i] != 0)
        {
            vals2[counter] = vals[i];
            counter += 1;
        }
    }
...
bool guess_valid(puzzle *puz)
{
    ...
    constraint c_1 = puz->constraints[0];
    char letter = c_1.letter;
    unsigned int number = c_1.number;
    char symbol = c_1.symbol;
    ...
    int *arr = malloc(sizeof(unsigned int) * count);
    int counter = 0;
    for (int i = 0; i < puz->boardSize; i++)
    {
        for (int j = 0; j < puz->boardSize; j++)
    ...
    {
        if (((arr[0] - arr[1]) == number) || ((arr[1] - arr[0]) == number))
        {
            free(arr);
            return true;
        }
...
puzzle apply_guess(puzzle *puz)
{
    ...
    // Complete your code here....
    // while (new.constraintsLen > 0)
    // {
    constraint c = new.constraints[0];
    constraint *newarr = malloc(sizeof(constraint) * (new.constraintsLen - 1));
    for (int i = 1; i < new.constraintsLen; i++)
    {
        newarr[i - 1] = new.constraints[i];
    }
    free(new.constraints);
    new.constraints = newarr;
    new.constraintsLen -= 1;

    for (int i = 0; i < new.boardSize; i++)
    {
        for (int j = 0; j < new.boardSize; j++)
        {
            if (new.board[i][j].g.letter == c.letter)
            {
                new.board[i][j].number = new.board[i][j].g.number;
...
puzArr neighbors(puzzle *puz)
{
    ...
    constraint c = puz->constraints[0];
    // printf("c %d\n", c.number);
    ...
    for (int i = 0; i < puz->boardSize; i++)
    {
        ...
        {
            entry val = puz->board[i][j];
            if (val.letter == c.letter)
            {
                if (!val.guess && val.number == 0)
                {
                    found = true;
                    posn temp = {j, i};
                    pos = temp;
    ...
    if (found)
    {
        // printf("%d, %d\n", pos.x, pos.y);
        //       printf("%d, %d\n", pos.x, pos.y);
        numArr vals = available_vals(puz, pos);
        ...
        for (int i = 0; i < vals.len; i++)
        {
            ...
            puzzle *temparr = malloc(sizeof(puzzle) * (nbrs.len + 1));
            for (int i = 0; i < nbrs.len; i++)
            {
                temparr[i] = nbrs.arr[i];
            }
    ...
    else
    {
        ...
        {
            puzzle new = puzzle_deep_copy(puz);
            puzzle out = apply_guess(&new);
            // printf("len %d\n", out.constraintsLen);
            // printf("%d\n", out.constraintsLen);
            // constraint *newarr = malloc(sizeof(constraint) * (out.constraintsLen - 1));
            // for (int i = 1; i < out.constraintsLen; i++)
            // {
            //     newarr[i - 1] = out.constraints[i];
            // }
            // // free(out.constraints);
            // out.constraints = newarr;
            // out.constraintsLen -= 1;
...
//         //     {
//         //         entry val = temp.board[i][j];

//         //         if ((val.letter == c.letter) && val.guess)
//         //         {
//         //             if (c.symbol == '+')
//         //             {
//         //                 c_reduced += val.g.number;
//         //             }
//         //             else if (c.symbol == '-')
//         //             {
//         //                 c_reduced = val.g.number;
